...
- `APITRACE_D3D12_RETRACE_PRESENT_DELAY_MS`：仅用于人工观察 D3D12 replay 窗口，不参与 trace
  语义或像素验收。
- `APITRACE_D3D12_RETRACE_STOP_AFTER_PRESENT_FRAME`：从 trace 开头完整 replay，到指定或更大的
  Present frame index 后停止。该变量不跳过前序帧，也不改变 capture 内容。
- `APITRACE_D3D12_RETRACE_STOP_AFTER_SEQUENCE`：从 trace 开头完整 replay，到指定或更大的
  D3D sequence 后停止。该变量不跳过前序命令。

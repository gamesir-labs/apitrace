...
cp "$runtime_path" "$TEST_PREFIX/bin/libwinpthread-1.dll"
...
for scene in $EXPECTED_SCENES; do
    if ! printf '%s\n' "$run_output" | grep -F "scene fail: $scene reason=$scene: dx12 scene is reserved but not implemented yet" >/dev/null; then
        echo "missing failure for scene: $scene" >&2
        printf '%s\n' "$run_output" >&2
        exit 1
    fi
done

if ! printf '%s\n' "$run_output" | grep -F "summary: passed=0 failed=7 skipped=0" >/dev/null; then

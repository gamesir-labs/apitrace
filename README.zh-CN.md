...
- `d3d11.dll` / `d3d12.dll` / `dxgi.dll`：给 Wine/Windows override 模式使用的 capture proxy DLL。
...
如果要覆盖 DX12 路径，则把 `d3d12.dll` 和 `dxgi.dll` 一并放到同目录，并追加对应
override。`d3d12.dll` 负责 D3D12 device/command/resource 语义，`dxgi.dll`
负责 swapchain creation 与 `IDXGISwapChain::Present/Present1`；只覆盖
`d3d12.dll` 时 trace 仍会缺少 DXGI Present。

...
class OrderedDispatchCompiler {
public:
  OrderedDispatchCompiler(fs::path output_path, std::size_t worker_count)
      : output_path_(std::move(output_path)),
        worker_count_(std::max<std::size_t>(1, worker_count)),
        max_outstanding_batches_(std::max<std::size_t>(2, worker_count_ * 2))
  {
  }

  ~OrderedDispatchCompiler()
  {
    cancel();
  }
  ...
  bool open(std::string &error)
  {
    error.clear();
    if (opened_) {
      error = "ordered dispatch compiler is already open";
      return false;
    }
    std::error_code directory_error;
    fs::create_directories(output_path_.parent_path(), directory_error);
    if (directory_error) {
      error = "failed to create compiled dispatch directory: " + directory_error.message();
      return false;
    }
    fs::remove(output_path_, directory_error);
    output_.open(output_path_, std::ios::binary | std::ios::trunc);
    if (!output_.is_open()) {
      error = "failed to create ordered compiled dispatch stream";
      return false;
    }
    apitrace::trace::CompiledDispatchHeader placeholder;
    if (!apitrace::trace::write_compiled_dispatch_header(output_, placeholder, error)) {
      output_.close();
      fs::remove(output_path_, directory_error);
      return false;
    }
  ...
  bool append_event(const apitrace::trace::EventRecord &event, std::string &error)
  {
    Input input;
    input.event = event;
    input.approximate_bytes = event.payload.size() + event.callsite.function_name.size() +
                              event.object_refs.size() * sizeof(std::uint64_t) +
                              event.blob_refs.size() * sizeof(std::uint64_t) + 128;
    current_batch_bytes_ += input.approximate_bytes;
    current_batch_.inputs.push_back(std::move(input));
    return flush_current_if_needed(error);
  }
  ...
  {
    Input input;
    input.serialized_line.assign(line.data(), line.size());
    input.source_offset = source_offset;
    input.approximate_bytes = input.serialized_line.size();
    current_batch_bytes_ += input.approximate_bytes;
    current_batch_.inputs.push_back(std::move(input));
    return flush_current_if_needed(error);
  }
      ...
      std::string &error)
  {
    error.clear();
    if (!opened_) {
      error = "ordered dispatch compiler is not open";
      return false;
    }
    if (!submit_current(error)) {
      cancel_with_error(error);
      return false;
    }
    while (outstanding_batches_ != 0) {
      if (!drain_next_result(true, error)) {
        cancel_with_error(error);
        return false;
      }
    }
    ...
    if (!output_.good() ||
        !apitrace::trace::write_compiled_dispatch_header(output_, header_, error)) {
      if (error.empty()) {
        error = "failed to patch ordered compiled dispatch header";
      }
      cancel_with_error(error);
      return false;
    }
    ...
    output_.close();
    if (!output_) {
      error = "failed to close ordered compiled dispatch stream";
      cancel_with_error(error);
      return false;
    }
    ...
    header = header_;
    opened_ = false;
    elapsed_ms_ = static_cast<std::uint64_t>(
  ...
  void cancel()
  {
    ...
    fs::remove(output_path_, remove_error);
    opened_ = false;
  }
...
private:
  ...
  bool flush_current_if_needed(std::string &error)
  {
    if (current_batch_bytes_ < kOrderedDispatchBatchBytes &&
        current_batch_.inputs.size() < kOrderedDispatchBatchRecords) {
      return drain_next_result(false, error);
    }
    return submit_current(error);
  }

  bool submit_current(std::string &error)
  {
    if (current_batch_.inputs.empty()) {
      return drain_next_result(false, error);
    }
    while (outstanding_batches_ >= max_outstanding_batches_) {
      if (!drain_next_result(true, error)) {
        return false;
      }
    }
    current_batch_.index = next_submit_index_++;
    current_batch_.approximate_bytes = current_batch_bytes_;
    {
      std::lock_guard<std::mutex> lock(mutex_);
      pending_batches_.push_back(std::move(current_batch_));
    }
  ...
  bool drain_next_result(bool wait, std::string &error)
  {
    ...
    Result result;
    {
      ...
      if (wait) {
        result_cv_.wait(lock, [&] {
          return completed_results_.find(next_write_index_) != completed_results_.end();
        });
      }
      const auto found = completed_results_.find(next_write_index_);
    ...
    if (!output_) {
      error = "failed to append ordered compiled dispatch batch";
      return false;
    }
    header_.record_count += result.record_count;
    header_.encoded_record_bytes += result.bytes.size();
    return true;
  }
  ...
  void worker_main()
  {
    for (;;) {
      ...
      {
        std::unique_lock<std::mutex> lock(mutex_);
        work_cv_.wait(lock, [&] { return stopping_ || !pending_batches_.empty(); });
      ...
      Result result;
      try {
        result.bytes.reserve(batch.approximate_bytes + batch.inputs.size() * 64);
        std::vector<std::uint8_t> encoded;
        for (auto &input : batch.inputs) {
          apitrace::trace::EventRecord event;
          if (input.serialized_line.empty()) {
            event = std::move(input.event);
          } else if (!apitrace::trace::parse_event_record_json_line(
                         input.serialized_line, event, result.error)) {
            result.error = "callstream byte " + std::to_string(input.source_offset) +
                           ": " + result.error;
            break;
          }
          if (!apitrace::trace::encode_compiled_dispatch_event(
                  event, encoded, result.error)) {
            const auto location = input.serialized_line.empty()
                                      ? "sequence " + std::to_string(event.callsite.sequence)
                                      : "callstream byte " + std::to_string(input.source_offset);
            result.error = location + ": " + result.error;
            break;
          }
      ...
      {
        std::lock_guard<std::mutex> lock(mutex_);
        completed_results_.emplace(batch.index, std::move(result));
      }
      result_cv_.notify_one();
    }
  }

  void stop_workers(bool discard_pending)
  {
    {
      std::lock_guard<std::mutex> lock(mutex_);
      stopping_ = true;
  ...
  void cancel_with_error(const std::string &)
  {
    cancel();
  }
  ...
  std::map<std::uint64_t, Result> completed_results_;
  Batch current_batch_;
  std::size_t current_batch_bytes_ = 0;
  std::size_t outstanding_batches_ = 0;
  std::uint64_t next_submit_index_ = 0;
  std::uint64_t next_write_index_ = 0;
  apitrace::trace::CompiledDispatchHeader header_;
  std::mutex mutex_;
    ...
    ProgressReporter *progress)
{
  ...
  // Raw blob references are published under provisional paths and therefore force a later
  // callstream rewrite to canonical asset paths. In that case compiling here would knowingly
  // create a stale multi-gigabyte dispatch stream. Defer once to the final object-audit scan.
  const bool compile_dispatch_from_raw = reader.blob_extents().empty();
  std::unique_ptr<OrderedDispatchCompiler> raw_dispatch_compiler;
  apitrace::trace::CompiledDispatchHeader raw_dispatch_header;
  std::string raw_dispatch_error;
  if (compile_dispatch_from_raw) {
    raw_dispatch_compiler = std::make_unique<OrderedDispatchCompiler>(
        temporary_dispatch_path, options.jobs);
    if (!raw_dispatch_compiler->open(raw_dispatch_error)) {
      std::cerr << "error: failed to start RAW dispatch compiler: "
                << raw_dispatch_error << "\n";
      writer.close();
      fs::remove(temporary_callstream_path, temporary_error);
      return false;
    ...
    stats.compiled_dispatch_source = "deferred-final-scan";
  }
  const auto append_raw_dispatch_event = [&](const apitrace::trace::EventRecord &event) {
    return !raw_dispatch_compiler ||
           raw_dispatch_compiler->append_event(event, raw_dispatch_error);
  };
  ...
  if (!ok) {
    flush_passthrough_lines();
    writer.close();
    if (raw_dispatch_compiler) {
      raw_dispatch_compiler->cancel();
    }
    std::error_code remove_error;
  ...
  stats.compiled_dispatch_ms = raw_dispatch_compiler->elapsed_ms();
...
std::uint64_t reference_rewrite_chunk_size()
{
#if defined(APITRACE_ENABLE_TEST_HOOKS)
  if (const char *value = std::getenv("APITRACE_TEST_REFERENCE_REWRITE_CHUNK_BYTES")) {
    char *end = nullptr;
    errno = 0;
    const auto parsed = std::strtoull(value, &end, 10);
...
struct JsonlReferenceRewriteContext {
  const std::unordered_map<std::string, std::string> &aliases;
  const std::unordered_map<std::string, std::uint64_t> &blob_id_by_effective_path;
  const std::unordered_map<std::uint64_t, std::string> &effective_path_by_blob_id;
...
struct JsonlReferenceRewriteLineResult {
  bool ok = true;
  bool changed = false;
  bool sanitized = false;
  bool recovered_any = false;
  bool remapped_blob_refs_file = false;
  bool primary_blob_ref_record = false;
...
JsonlReferenceRewriteLineResult rewrite_jsonl_reference_line(
    const JsonlLineView &line_view,
    const JsonlReferenceRewriteContext &context,
    bool collect_referenced_paths)
{
  JsonlReferenceRewriteLineResult result;
  const auto line = line_view.line;
  const auto tokens = scan_jsonl_line_tokens(line);
  ...
  const auto path_candidates = may_have_path_reference
      ? scan_jsonl_path_rewrite_candidates(
            line,
            context.aliases,
            context.blob_id_remap_paths,
            !context.effective_path_by_blob_id.empty() && tokens.asset_token,
            !context.blob_id_remap_by_path.empty() && tokens.blob_refs_key)
  ...
  if (record.is_discarded()) {
    if (may_rewrite_blob_refs || context.options.verify_jsonl_records) {
      bool recovered_any = false;
      for (const auto &recovered : recover_embedded_jsonl_records(line_text)) {
        ...
        const bool recovered_blob_refs_changed = rewrite_blob_refs(recovered_record, context.blob_id_remap);
        result.output_lines.push_back(recovered_blob_refs_changed ? recovered_record.dump() : recovered);
        if (recovered_blob_refs_changed) {
          result.remapped_blob_refs_file = true;
          result.primary_blob_ref_record = true;
          result.changed = true;
          ++result.rewritten_records;
        }
        recovered_any = true;
      }
      result.sanitized = true;
      result.changed = true;
      result.recovered_any = recovered_any;
      if (!recovered_any) {
  ...
  const bool paths_changed = rewrite_path_refs(record, context.aliases);
  const bool single_blob_asset_alias_changed =
      rewrite_single_blob_asset_alias_refs(record, context.effective_path_by_blob_id);
  const bool blob_refs_changed =
      rewrite_blob_refs_for_effective_paths(
          record,
          context.blob_id_by_effective_path,
          paths_changed || single_blob_asset_alias_changed);
  const bool path_mapped_blob_refs_changed =
      rewrite_blob_refs_by_referenced_paths(record, context.blob_id_remap_by_path);
  const bool remapped_blob_refs_changed = rewrite_blob_refs(record, context.blob_id_remap);
  result.remapped_blob_refs_file = remapped_blob_refs_changed;
  if (paths_changed ||
      single_blob_asset_alias_changed ||
      blob_refs_changed ||
      path_mapped_blob_refs_changed ||
      remapped_blob_refs_changed) {
    if (collect_referenced_paths) {
      collect_path_references_from_json(record, context.aliases, result.referenced_paths, false);
    }
    result.output_lines.push_back(record.dump());
    result.changed = true;
    ++result.rewritten_records;
    result.primary_blob_ref_record = blob_refs_changed || path_mapped_blob_refs_changed || remapped_blob_refs_changed;
  } else if (collect_referenced_paths) {
    collect_path_references_from_json(record, context.aliases, result.referenced_paths, false);
  }
  return result;
}

struct JsonlReferenceRewriteChunkResult {
  bool ok = true;
  bool changed = false;
  bool sanitized = false;
  bool remapped_blob_refs_file = false;
...
bool write_original_jsonl_line_from_chunk(
    ...
    HashedOutputFile &output)
{
  if (line_view.has_newline && line_view.byte_size == line_view.line.size() + 1) {
    output.write_text(std::string_view(
        chunk_text.data() + static_cast<std::size_t>(line_view.offset - chunk.offset),
...
bool write_reference_rewritten_chunk(
    const fs::path &jsonl_path,
    const JsonlByteChunk &chunk,
    const JsonlReferenceRewriteContext &context,
    bool collect_referenced_paths,
    HashedOutputFile &output,
    JsonlReferenceRewriteChunkResult &result)
{
  std::string text;
  if (!read_jsonl_byte_chunk(jsonl_path, chunk, text)) {
    result.ok = false;
    return false;
  }
  ...
  std::uint64_t copied_until = chunk.offset;
  auto copy_unchanged_prefix = [&](std::uint64_t end_offset) -> bool {
    if (end_offset <= copied_until) {
      return true;
    }
    output.write_text(std::string_view(
        text.data() + static_cast<std::size_t>(copied_until - chunk.offset),
        static_cast<std::size_t>(end_offset - copied_until)));
    copied_until = end_offset;
    return true;
  };
  for_each_jsonl_line_in_chunk(text, chunk.offset, [&](const JsonlLineView &line_view) {
    ++result.stats.jsonl_records;
    result.stats.input_bytes += line_view.byte_size;
    const auto rewritten = rewrite_jsonl_reference_line(line_view, context, collect_referenced_paths);
    if (!rewritten.ok) {
      result.ok = false;
      ok = false;
      return false;
    ...
    if (collect_referenced_paths) {
      result.referenced_paths.insert(rewritten.referenced_paths.begin(), rewritten.referenced_paths.end());
    }
    if (rewritten.sanitized) {
      result.sanitized = true;
      result.dropped_lines += rewritten.dropped_lines;
    }
    result.remapped_blob_refs_file = result.remapped_blob_refs_file || rewritten.remapped_blob_refs_file;
    if (rewritten.changed) {
      result.changed = true;
      if (!output_started) {
        if (!copy_unchanged_prefix(line_view.offset)) {
          result.ok = false;
          ok = false;
          return false;
        }
      ...
      for (const auto &output_line : rewritten.output_lines) {
        output.write_line(output_line);
        note_jsonl_output(result.stats, output_line);
      }
      copied_until = line_view.offset + line_view.byte_size;
      result.stats.rewritten_records += rewritten.rewritten_records;
      if (rewritten.primary_blob_ref_record) {
        ++result.stats.rewritten_primary_blob_ref_records;
      }
      return true;
    }
    if (output_started) {
      write_original_jsonl_line_from_chunk(text, chunk, line_view, output);
      copied_until = line_view.offset + line_view.byte_size;
    }
    note_jsonl_output(result.stats, line_view.line);
    return true;
  });
  if (ok && !output_started) {
    output.write_text(text);
  } else if (ok && copied_until < chunk.offset + chunk.size) {
    copy_unchanged_prefix(chunk.offset + chunk.size);
  }
  return ok && result.ok;
}

bool rewrite_jsonl_references_parallel(
    const fs::path &bundle_root,
    const fs::path &relative,
    const JsonlReferenceRewriteContext &context,
    Stats &stats,
...
{
  const auto absolute = bundle_root / relative;
  auto chunks = build_newline_aligned_jsonl_chunks(absolute, reference_rewrite_chunk_size());
  if (chunks.empty()) {
    return false;
  }
  ...
  bool changed = false;
  ++stats.jsonl_passes;
  std::vector<JsonlReferenceRewriteChunkResult> chunk_results(chunks.size());
  ...
  if (!context.options.dry_run) {
    ...
    const auto rewrite_worker = [&]() {
      for (;;) {
        ...
        const bool ok = write_reference_rewritten_chunk(
            absolute,
            chunks[index],
            context,
            referenced_paths != nullptr,
            chunk_output,
            chunk_results[index]);
        chunk_output.close();
        if (!ok || !chunk_results[index].ok) {
          std::lock_guard<std::mutex> lock(rewrite_error_mutex);
          rewrite_failed = true;
          return;
        }
    ...
    std::vector<std::thread> threads;
    const auto thread_count = std::max<std::size_t>(1, std::min(context.options.jobs, chunks.size()));
    threads.reserve(thread_count);
    for (std::size_t i = 0; i < thread_count; ++i) {
      threads.emplace_back(rewrite_worker);
    }
    for (auto &thread : threads) {
      thread.join();
    }
    ...
    for (std::size_t index = 0; index < chunks.size(); ++index) {
      ...
      changed = changed || chunk_results[index].changed;
      if (referenced_paths) {
        referenced_paths->insert(chunk_results[index].referenced_paths.begin(), chunk_results[index].referenced_paths.end());
      }
    ...
    for (std::size_t index = 0; index < chunks.size(); ++index) {
      if (!write_reference_rewritten_chunk(
              absolute,
              chunks[index],
              context,
              referenced_paths != nullptr,
  ...
  for (std::size_t index = 0; index < chunks.size(); ++index) {
    stats.jsonl_records += chunk_results[index].stats.jsonl_records;
    stats.input_bytes += chunk_results[index].stats.input_bytes;
    stats.output_bytes += chunk_results[index].stats.output_bytes;
    stats.rewritten_records += chunk_results[index].stats.rewritten_records;
    stats.rewritten_primary_blob_ref_records += chunk_results[index].stats.rewritten_primary_blob_ref_records;
  }
  ...
  bool remapped_blob_refs_file = false;
  bool sanitized_file = false;
  for (const auto &chunk_result : chunk_results) {
    remapped_blob_refs_file = remapped_blob_refs_file || chunk_result.remapped_blob_refs_file;
    sanitized_file = sanitized_file || chunk_result.sanitized;
    stats.dropped_jsonl_lines += chunk_result.dropped_lines;
  }
  if (sanitized_file) {
  ...
  if (!context.options.dry_run) {
    ...
    output.close();
    if (replace_with_temporary_file(absolute, temporary) && digest_cache && rewritten_digests) {
    ...
    std::unordered_map<std::string, std::pair<std::string, std::uint64_t>> *rewritten_digests = nullptr)
{
  ...
  for (const auto &relative : reference_files) {
    ...
    json root;
    if (relative.extension() == ".jsonl") {
      if (options.jobs > 1 && !options.verify_jsonl_records) {
        JsonlReferenceRewriteContext context{
            aliases,
            ...
            blob_id_remap_paths,
            options};
        const auto before_rewritten_text_files = stats.rewritten_text_files;
        if (rewrite_jsonl_references_parallel(
                bundle_root,
                relative,
                context,
                stats,
                ...
                digest_cache,
                rewritten_digests) &&
            stats.rewritten_text_files != before_rewritten_text_files) {
...
bool repair_missing_d3d12_device_objects(
    const fs::path &bundle_root,
    const Options &options,
    Stats &stats,
    FileDigestCache &digest_cache,
    std::unordered_map<std::string, std::pair<std::string, std::uint64_t>> &rewritten_digests)
{
  ...
  std::unique_ptr<OrderedDispatchCompiler> dispatch_compiler;
  ...
  if (compile_dispatch_during_scan) {
    dispatch_compiler = std::make_unique<OrderedDispatchCompiler>(
        temporary_dispatch_path, options.jobs);
    if (!dispatch_compiler->open(dispatch_error)) {
      std::cerr << "error: failed to start final-scan dispatch compiler: "
                << dispatch_error << "\n";
      return false;
    }
  }
  ...
  const bool scanned = scan_jsonl_file(callstream_path, [&](const JsonlLineView &line_view) {
    ...
    if (compile_dispatch_during_scan && line_view.offset != 0 && !line.empty()) {
      if (!dispatch_compiler->append_json_line(
              line, line_view.offset, dispatch_error)) {
        return false;
      }
    }
...
int apitrace::tools::run_bundle_finalize(int argc, char **argv){
  ...
  run_stage(options, progress, stage_index, kStageCount, "repair_object_index", [&] {
    if (!repair_missing_d3d12_device_objects(
            options.bundle_root,
            options,
            stats,
            digest_cache,

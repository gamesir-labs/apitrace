    ...
    std::string &error)
{
  if (relative_path_text.empty()) {
    error = event_label + ": " + description + " is missing source_asset_path";
    return false;
  }
  const std::filesystem::path relative_path(relative_path_text);
  if (!path_is_safe(relative_path)) {
    error = event_label + ": " + description + " has unsafe source_asset_path";
    return false;
  }
  std::error_code stat_error;
  const auto actual_size = std::filesystem::file_size(reader.layout().root_path / relative_path, stat_error);
  if (stat_error) {
    error = event_label + ": " + description + " source_asset_path is missing";
    return false;
  }
  if (actual_size < required_size) {
    error = event_label + ": " + description + " source_asset_path is truncated";
    return false;
  }
  return true;
}
    ...
    std::string &error)
{
  const auto require_integer_field = [&](const char *field) -> bool {
    ...
    if (it == payload.end() || (!it->is_number_unsigned() && !it->is_number_integer())) {
      error = event_label + ": buffer update is missing " + field;
      return false;
    }
    return true;
  };
  ...
  if (length == 0 || source_asset_size != length || source_offset != offset) {
    error = event_label + ": buffer update range does not match captured asset";
    return false;
  }
  const auto source_asset_path = string_from_json(payload.value("source_asset_path", json(nullptr)));
  return asset_file_has_bytes(reader, event_label, source_asset_path, source_asset_size, "buffer update", error);
}
    ...
    std::string &error)
{
  const auto require_integer_field = [&](const char *field) -> bool {
    const auto it = payload.find(field);
    if (it == payload.end() || (!it->is_number_unsigned() && !it->is_number_integer())) {
      error = event_label + ": texture update is missing " + field;
      return false;
    }
    return true;
  };
  const auto require_u64_triplet = [&](const char *field, bool positive) -> bool {
    ...
    if (it == payload.end() || !it->is_array() || it->size() != 3) {
      error = event_label + ": texture update has invalid " + field;
      return false;
    }
    for (std::size_t index = 0; index < 3; ++index) {
      ...
      if (positive && json_u64((*it)[index]) == 0) {
        error = event_label + ": texture update has zero " + field;
        return false;
      }
    }
    return true;
  ...
  if (bytes_per_row == 0 || source_asset_size == 0) {
    error = event_label + ": texture update has invalid byte layout";
    return false;
  }
  ...
  if ((depth > 1 && bytes_per_image == 0) || expected_size == 0 || source_asset_size < expected_size) {
    error = event_label + ": texture update asset is smaller than the described region";
    return false;
  }
  const auto source_asset_path = string_from_json(payload.value("source_asset_path", json(nullptr)));
  return asset_file_has_bytes(reader, event_label, source_asset_path, source_asset_size, "texture update", error);
}
    ...
    std::string &error)
{
  ...
  for (const auto &event : reader.metal_events()) {
    ...
    } else if (event.call_kind == trace::MetalCallKind::BufferUpdate) {
      auto buffer_id = object_id_field(payload, "buffer_id");
      if (buffer_id == 0) {
        buffer_id = event.object_id;
      }
      if (!require_metal_object(buffer_ids, buffer_id, event_label,
              "buffer update references an unknown buffer", error)) {
        return false;
      }
      if (!validate_metal_buffer_update_payload(reader, event_label, payload, error)) {
        return false;
      }
    } else if (event.call_kind == trace::MetalCallKind::TextureUpdate) {
      auto texture_id = object_id_field(payload, "texture_id");
      if (texture_id == 0) {
        texture_id = event.object_id;
      }
      if (!require_metal_object(texture_ids, texture_id, event_label,
              "texture update references an unknown texture", error)) {
        return false;
      }
      if (!validate_metal_texture_update_payload(reader, event_label, payload, error)) {
        return false;
      }

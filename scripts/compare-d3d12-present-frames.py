#!/usr/bin/env python3
"""Compare D3D12PresentFrame debug assets from two trace bundles."""
    ...
    with callstream.open("r", encoding="utf-8") as stream:
        for line_number, line in enumerate(stream, 1):
            ...
            if record.get("record_kind") != "resource_blob":
                continue
            if record.get("debug_name") != "D3D12PresentFrame":
                continue
            payload = record.get("payload")
            if not isinstance(payload, dict):
                fail(f"{callstream}:{line_number}: D3D12PresentFrame missing payload")
            if payload.get("format") != "rgba8":
                fail(f"{callstream}:{line_number}: unsupported frame format {payload.get('format')!r}")
            try:
                frame = Frame(
                ...
                fail(f"{callstream}:{line_number}: D3D12PresentFrame missing {exc.args[0]}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare D3D12PresentFrame debug assets from two apitrace bundles."
    )
...
def main() -> int:
    ...
    if not expected_frames:
        fail(f"{args.expected_bundle} has no D3D12PresentFrame assets")
    if not actual_frames:
        fail(f"{args.actual_bundle} has no D3D12PresentFrame assets")

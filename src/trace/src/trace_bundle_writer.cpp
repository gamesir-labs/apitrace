    ...
    for (;;) {
      ...
      if (can_hash_address) {
        ...
        if (existing_file_matches_content_hash(final_path, digest, job.payload->size(), existing_byte_size, &size_mismatch)) {
          if (job.target_path != final_path)
            std::filesystem::remove(job.target_path);
          ...
          if (std::filesystem::exists(job.target_path) && final_path != job.target_path) {
            std::error_code error;
            std::filesystem::rename(job.target_path, final_path, error);
            if (error) {
...
struct TraceBundleWriter::Impl {
  ...
  AsyncLineWriter metal_callstream_stream;
  ...
  std::size_t exact_dedup_compare_threshold = 16ull * 1024ull * 1024ull;
  ...
  std::vector<std::filesystem::path> rewrite_candidates() const
  {
    std::vector<std::filesystem::path> candidates;
    std::unordered_set<std::string> seen;
    if (callstream_may_reference_asset_alias)
      add_rewrite_candidate(candidates, seen, layout.callstream_path);
    if (metal_callstream_may_reference_asset_alias)
...
bool TraceBundleWriter::open(const std::filesystem::path &bundle_root, TraceBundleOpenMode mode)
{
  ...
  if (mode == TraceBundleOpenMode::Primary) {
    ...
    if (std::filesystem::is_regular_file(impl_->layout.callstream_path)) {
      impl_->callstream_stream.open(impl_->layout.callstream_path, std::ios::app);
...
void TraceBundleWriter::append_metal_event(const MetalEventRecord &event)
{
  ...
  if (!impl_->metal_callstream_stream.is_open()) {
    impl_->metal_callstream_stream.open(impl_->layout.metal_callstream_path, std::ios::trunc);
  }
  auto line = detail::metal_event_record_json(event);
  if (line.find("asset-") != std::string::npos)
    impl_->metal_callstream_may_reference_asset_alias = true;
  impl_->metal_callstream_stream.write_line(std::move(line));
}
...
AssetRecord TraceBundleWriter::register_asset(AssetRecord &&asset)
{
  ...
  if (!fingerprint_key.empty()) {
    std::lock_guard lock(impl_->asset_mutex);
    if (payload->size() > impl_->exact_dedup_compare_threshold)
    ...
    if (payload->size() < impl_->async_asset_threshold) {
      const auto known_hash = impl_->content_hash_by_fast_fingerprint.find(fingerprint_key);
      if (known_hash != impl_->content_hash_by_fast_fingerprint.end()) {
        ++impl_->asset_sync_hashes;
        impl_->asset_sync_hash_bytes += payload->size();
        const auto candidate_hash = sha256_bytes(*payload);
        if (candidate_hash == known_hash->second) {
          const auto known_key = asset_content_key(finalized.kind, known_hash->second);
          if (const auto existing = impl_->assets_by_content_hash.find(known_key);
              existing != impl_->assets_by_content_hash.end()) {
            finalized.content_hash = known_hash->second;
          }
        } else {
    ...
    if (payload->size() >= impl_->async_asset_threshold) {
      ...
      if (impl_->asset_writer.enqueue(
              ...
              [impl = impl_.get(), fingerprint_key, finalized, payload]() {
                if (!fingerprint_key.empty()) {
                  std::lock_guard lock(impl->asset_mutex);
                  impl->pending_assets_by_fast_fingerprint[fingerprint_key].push_back(PendingAssetEntry{
                      finalized,
                      payload->size() <= impl->exact_dedup_compare_threshold ? payload : nullptr,
                  });
                }
              })) {
...
AssetRecord TraceBundleWriter::register_metal_asset(MetalAssetKind kind, AssetRecord &&asset)
{
  ...
  if (!fingerprint_key.empty()) {
    ...
    if (payload->size() < impl_->async_asset_threshold) {
      const auto known_hash = impl_->metal_content_hash_by_fast_fingerprint.find(fingerprint_key);
      if (known_hash != impl_->metal_content_hash_by_fast_fingerprint.end()) {
        ++impl_->asset_sync_hashes;
        impl_->asset_sync_hash_bytes += payload->size();
        const auto candidate_hash = sha256_bytes(*payload);
        if (candidate_hash == known_hash->second) {
          const auto known_key = metal_asset_content_key(kind, finalized.kind, known_hash->second);
    ...
    if (payload->size() >= impl_->async_asset_threshold) {
      ...
      if (impl_->asset_writer.enqueue(
              ...
              [impl = impl_.get(), fingerprint_key, finalized, payload]() {
                if (!fingerprint_key.empty()) {
                  std::lock_guard lock(impl->asset_mutex);
                  impl->pending_metal_assets_by_fast_fingerprint[fingerprint_key].push_back(PendingAssetEntry{
                      finalized,
                      payload->size() <= impl->exact_dedup_compare_threshold ? payload : nullptr,
                  });
                }
...
void TraceBundleWriter::close()
{
  ...
  const auto async_path_aliases = impl_->asset_writer.completed_path_aliases();
  ...
  if (impl_->writer_stats) {
    ...
    auto stats_relative_paths =
        (impl_->open_mode == TraceBundleOpenMode::SidebandOnly || env_flag_enabled("APITRACE_CHECKSUM_FULL_SCAN"))
            ? collect_bundle_relative_paths_full_scan(impl_->layout)
            : collect_writer_known_relative_paths(impl_->layout, known_file_digests, rewritten_paths);
  ...
  for (const auto &relative_path : relative_paths) {
    ...
    } else if (const auto existing = existing_records.find(key); existing != existing_records.end()) {
      file_record = existing->second;
    } else {
